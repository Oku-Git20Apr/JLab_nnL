...
void all_pdf(){
  ...
  TH2F *h2 = new TH2F("h2","h2",1000,-1,1,1000,0,0.6);
  h2->SetStats(kFALSE);
  SetTH2(h2,"","","",0.020,10);
  TGraph *gr[200];
...
0.0100699};
  ...
  SetGr(gr7,"","","",6,6,1,1.0);//Pink for Abst.
  gr7->SetLineWidth(2);
  gr7->SetLineStyle(1);
...
0.066
  ...
  TLegend *leg = new TLegend(0.50,0.70,0.98,0.98,"","NDC");
  leg->SetFillColor(0);
  ...
  leg->SetNColumns(1);
  leg->SetMargin(0.15);
  leg->AddEntry(gr3     ,Form("SAPHIR #scale[0.7]{EPJA19(2004) 251.}"
  ...
  leg->AddEntry(gr1     ,Form("CLAS  #scale[0.7]{PRC81(2010) 025201.}"
                        ,dataset1->GetKinematics()->GetW()/1000.),"pl");
  //leg->AddEntry(gr8     ,"LEPS #scale[0.6]{PRC73(2006) 035214}        #scale[0.5]{(#sqrt{s}=2.163 GeV)}","pl");
  leg->AddEntry(gr5     ,"SLA    #scale[0.7]{PRC58(1998) 75.}","pl");
  leg->AddEntry(gr4     ,"KM     #scale[0.7]{PRC61(1999) 012201.}","pl");
  //leg->AddEntry(gr6     ,"RPR3  #scale[0.6]{PRC73(2006) 045207.}      #scale[0.5]{(#sqrt{s}=2.130 GeV)}","pl");
  leg->AddEntry(gr7     ,"RPR2011 #scale[0.7]{PRL108(2012) 182002.}","pl");
...
//	ye3_result[2]=0.023;
  ...
  //TGraph *gr_result = new TGraphAsymmErrors(1, x_result, y_result);
  ...
  //TGraph *gr2_result = new TGraphAsymmErrors(2, x2_result, y2_result);
...
c1->Print("/data/41a/ELS/okuyama/JLab_nnL/okuya_macros/mthesis_Fig/pdf/abst_CS_thetadepL.pdf");
}
