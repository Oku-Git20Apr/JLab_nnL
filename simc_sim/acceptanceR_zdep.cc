...
int main(int argc, char** argv){
  ...
  gStyle->SetTitleOffset(0.9, "Y");
	...
	//TH1D *h_z_result = new TH1D("h_z_result", "", bin_mom, -15., 15.);
	//t->Project("h_z_result","zposi");
...
//----------------------------//
    ...
    tree->SetBranchStatus("zposi"    ,1);tree->SetBranchAddress("zposi"    ,&z     );
	tree->SetBranchStatus("Rp_orig",1);tree->SetBranchAddress("Rp_orig",&mom_acpt);
  ...
  for(int i=0;i<ENum;i++){
	...
	double eps=1/(1+2*(q2/Qsq)*tan(theta/2)*tan(theta/2));
		...
		//h_mom_gen->SetBinContent(i,2549979*(1./189.)*(max_mom-min_mom)/bin_mom);//RHRS 1,000,000 (2020/10/4) Z uniform
		//h_mom_gen->SetBinContent(i,175296/1000000.*2549979*(1./189.)*(max_mom-min_mom)/bin_mom);//RHRS 1,000,000 (2020/10/4) Z > 8cm
		h_mom_gen->SetBinContent(i,167359/1000000.*2549979*(1./189.)*(max_mom-min_mom)/bin_mom);//RHRS 1,000,000 (2020/10/4) Z < -8cm
		}
	double vpflux=Escat*kg/(137*2*PI*PI*Einc*Qsq*(1-eps));
	double k = MAX*gRandom->Uniform();
		//h_mom_result->SetBinContent(h_mom_gen->FindBin(mom),mom);
		if(z<-8.)h_mom_result->Fill(mom_acpt);
//cout<<"Qsq="<<Qsq<<endl;
//cout<<"q2="<<q2<<endl;
//cout<<"kg="<<kg<<endl;
//cout<<"eps="<<eps<<endl;
