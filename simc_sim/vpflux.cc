...
int main(int argc, char** argv){
  ...
  gStyle->SetTitleOffset(0.9, "Y");
	...
	double min_mom = 1800;//nnL
	double max_mom = 2400;//nnL
...
//----------------------------//
	...
	tree->SetBranchStatus("Lp_gen",1);tree->SetBranchAddress("Lp_gen",&mom);
	tree->SetBranchStatus("Lth_gen",1);tree->SetBranchAddress("Lth_gen",&th);
    tree->SetBranchStatus("Lph_gen"    ,1);tree->SetBranchAddress("Lph_gen"    ,&ph     );
  ...
  for(int i=0;i<ENum;i++){
	...
	double eps=1/(1+2*(q2/Qsq)*tan(theta/2)*tan(theta/2));
		...
		h_mom_gen->SetBinContent(i,26147*(1./189.)*(max_mom-min_mom)/bin_mom);
		}
	double vpflux=Escat*kg/(137*2*PI*PI*Einc*Qsq*(1-eps));
