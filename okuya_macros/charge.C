...
//============  Reading the Rootfile =======================//
  ...
  const TString rootfilePath = "/data/41a/ELS/JLab/E12-17-003/root/";//farm
  ...
  ofstream ofp2("current.txt",ios::app);
  for (Int_t i = 0; i<evnentries ; i++) {
    ev->GetEntry(i);
    //========current
    I_d3[i]   = g[0]*d3r   + of[0];
    ...
    I_unew[i] = g[2]*unewr + of[2];
    I_dnew[i] = g[3]*dnewr + of[3];
  //cout << "time " << clk/103700.0 << endl;
  //cout<<"eventID: "<<i<<endl;
  //cout<<"I_ave="<<(I_d3[i]+I_unew[i]+I_dnew[i])/3.<<endl;
  ofp2<<Form("%.1lf %.4lf",clk/103700.0,(I_d3[i]+I_unew[i]+I_dnew[i])/3.)<<endl;
  }
