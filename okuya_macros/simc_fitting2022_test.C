...
// Convolution integral
  ...
  val2 =  step * sum * invsq2pi / (par[3]*par[4]*exp(xx*par[5]/par[4]));
...
//bg_only_strict
	...
	TCanvas *c11 = new TCanvas("c11","c11",800,800);
	 hmm_simcL2->Draw("");
	 fmm_Lambda_only_strict->Draw("same");
	 hmm_simcL2->Fit("fmm_Lambda_only_strict","","",fit_min_mm,fit_max_mm);//Total fitting (full) w/ 4Poly BG
	...
	TCanvas *c12 = new TCanvas("c12","c12",800,800);
	 hmm_simcS2->Draw("");
	 fmm_Sigma_only_strict->Draw("same");
	 hmm_simcS2->Fit("fmm_Sigma_only_strict","","",fit_min_mm,fit_max_mm);//Total fitting (full) w/ 4Poly BG
