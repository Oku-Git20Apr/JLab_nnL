...
//SIMC Event Loop//
  ...
  double cs = 0., cs_tri=0., cs_lab = 0.;//cs(in CM), cs(for tripleDCS), cs(triple, all lab)
...
//cout<<"theta_gk_cm="<<theta_gk_cm_test*180./PI<<endl;
		...
		int kbinR = (int)((L_R_mom-1.6)*100./0.4);
		int kbinL = (int)((L_L_mom-1.9)*100./0.4);
		...
		if(RHRS!=0.){
			cs = labtocm/RHRS;//[nb/sr]
			if(LHRS!=0.){	
			cs_tri = labtocm/LHRS/RHRS;//[nb/sr/sr]
			cs_lab = 1./LHRS/RHRS;//[nb/sr/sr]
			}else{cs_tri=0.;cs_lab=0.;}
		}else{cs=0.;}
...
//FILL into histograms//
	...
	//change
		if(L_momR>1.760&&L_momR<1.900&&L_momL>2.010&&L_momL<2.160){
			...
			hcs1_simcL->SetBinContent(hcs1_simcL->FindBin(ran*1000.),cs);
			hcs2_simcL->SetBinContent(hcs2_simcL->FindBin(ran*1000.),cs_tri);
			hcs3_simcL->SetBinContent(hcs3_simcL->FindBin(ran*1000.),cs_lab);
...
//change
	...
	//hmm_simcS->Scale(60.8617*60.8617/77206./65.1809);//2021Apr.//Qsq2_2
