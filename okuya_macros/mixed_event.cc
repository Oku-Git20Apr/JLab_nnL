/*
  mixed_event.cc
  Missing mass reconstuction w/ mixed event analysis
...
const double m_3He = 2.80839133; // He3 mass
const double m_dL = 1.875612762 + 1.115683; // d+L
const double m_3H   = 2.80892086; // 3H
const double m_nnL = 0.939565379*2.0 + 1.115683; // nn+L

//double mt = mp; double mh = mL; // p(e,e'K+)L
//double mt = m_3He; double mh = m_dL; // 3He(e,e'K+)3LH
double mt = m_3H; double mh = m_nnL; // 3H(e,e'K+)nnL
...
void mixed_event(){
  //TFile* f1 = new TFile("../h2_20191212.root"); mt=mp; mh = mL;
  //TFile* f1 = new TFile("./h22_20200121.root"); mt=mp; mh = mL;
  TFile* f1 = new TFile("./fout.root"); mt=m_3H; mh = m_nnL;
  TTree* t1 = (TTree*)f1->Get("tree_out");
  Double_t trig5;
  Double_t trig4;
  Double_t trig1;
  ...
  h2->GetXaxis()->SetRangeUser(-100.0,190.0);
  h2->SetLineColor(1);
  TH1F* h2_acc   = (TH1F*)h2->Clone("h2_acc");
  TH1F* h2_acc_org   = (TH1F*)h2->Clone("h2_acc_org");
  ...
  for(int i=0 ; i<ent ; i++){
    ...
    if(a2 > 1.0 && a2 <23.0 ) ac2_selection = true;
	...
	//dpk  = 3.158e-2* par_k[2]  + 0.4058;// MeV/c
      ...
      mm = CalcMM(hallap, par_ep, par_k, mt);
      mm   = (mm-mh)*1000.; // Gen --> MeV
	...
	for(int j=0 ; j<(int)nmix ; j++){
	  ...
	  if(a1<1.0 && a2>1.0 && a2<23.0
	     && fabs(rvz[0]-lvz[0])<0.020
	  ...
	  mixed_mm = CalcMM(hallap, par_ep, par_k, mt);
	  //mixed_mm = (mixed_mm - mL) * 1000.; // Gen --> MeV
	  mixed_mm = (mixed_mm - mh) * 1000.; // Gen --> MeV
...
//			    h2_H_acc->GetBinError(i+1)*mixscale);
  ...
  //h2->Draw();
  double ntemp1 = h2_acc_org->GetEntries();
  double ntemp2 = h2_acc->GetEntries();
  //cout << " SCALE: " << ntemp1/ntemp2/5. << endl;
  h2_acc_org->Scale(1./5.);
  h2_acc->Draw("h");
  h2_acc->SetMarkerStyle(1);
  h2_acc->Scale(ntemp1/ntemp2/5.0);
  h2_acc_org->Draw("same");
  ...
  cout << " The number of events: " << endl;
  double allnum = h2->Integral(0,xbin);
  double accnum = h2_acc2->Integral(0,xbin);
  cout << " " << allnum << " - " << accnum << " = " << allnum - accnum << endl;
