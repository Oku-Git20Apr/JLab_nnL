...
cout << "Output pdf file name is " << pdfname << endl;
  ...
  TFile *file_new = new TFile("bgmea_pion.root","recreate");//new root
...
/***********************************/
 ...
 // tree->Draw(">>elist", "abs(ct_orig+7*2.0)<1.0||abs(ct_orig+2*2.0)<1.0||abs(ct_orig+1.0*2.0)<1.0||abs(ct_orig-4.0*2.0)<1.0||abs(ct_orig-5.0*2.0)<1.0||abs(ct_orig-6.0*2.0)<1.0");
  tree->Draw(">>elist", "abs(ct_orig-3.0)<1.0");
...
//----MM Calc. again----//
  ...
  cout<<nmix<<" x 1 x 21.54 bunches"<<"= "<<nmix * 1.0 * 21.54 <<" (" << "effective scale="<<1/mixscale<<") <-- best"<<endl;
...
cout<<"6 bunches used "<<"( effective scale = "<<ntemp4/ntemp3<<" )"<<endl;
	...
	hmm_mixacc2->Scale(1./1./21.54);
	hmm_mixacc2->Draw("same");
	
	//MM + BG(MEA)
  TCanvas* c7 = new TCanvas("c7","MM + BG(MEA)");
  hmm_best->Draw();
  hmm_mixacc3->Scale(1./1./nmix/21.54);
