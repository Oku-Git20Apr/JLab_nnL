...
//   max_coin_c=1000.0;
 ...
 max_adc=20000.;
	...
	//=== AC Threshold variable ===//
     ...
     //----------------NPE---------------//
	 chi2[0]=0.;
	 for(int i=1; i<nth; i++) chi2[i]=chi2[i-1]+0.0001;
	 zver[0]=0.000;
	 //for(int i=1; i<nth; i++) zver[i]=zver[i-1]+0.01;//SUM
	 for(int i=1; i<nth; i++) zver[i]=zver[i-1]+0.001;//Diff
	 th1_max=2.0;
...
////--------------------------------------------//
	  h_pisr1 = new TH1F("h_pisr1","Pion Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_ksr1 = new TH1F("h_ksr1","Kaon Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_psr1 = new TH1F("h_psr1","Proton Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_Lsr1 = new TH1F("h_Lsr1","Lambda Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_Ssr1 = new TH1F("h_Ssr1","Signa Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_pitot1 = new TH1F("h_pitot1","Pion Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_ktot1 = new TH1F("h_ktot1","Kaon Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_ptot1 = new TH1F("h_ptot1","Proton Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_Ltot1 = new TH1F("h_Ltot1","Lambda Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
	  h_Stot1 = new TH1F("h_Stot1","Sigma Survival Ratio",nth-1,chi2[0],chi2[nth-1]);
//--------------------------------------------//
	  h_pisr1->Sumw2();
...
/////////////////////
 ...
 // h_m2_mm    = new TH2D("h_m2_mm"   ,"h_m2_mm"   ,bin_mm,min_mm,max_mm,400,0.,20.); 
  ...
  h_zz_test    = new TH2D("h_zz_test"   ,"h_zz_test"   , 400,-0.25,0.25,400,  -0.25, 0.25); 
...
//			for (int l=0;l<nth;l++){
				int j=0; int l=0;
				hcoin_k_fom[i][j][l]=new TH1F(Form("hcoin_k_fom[%d][%d][%d]",i,j,l), Form("Cointime (Kaon) (zR+zL)/2<%lf  cut",chi2[i]),bin_coin_c,min_coin_c,max_coin_c);
				set->SetTH1(hcoin_k_fom[i][j][l],Form("Cointime (Kaon) (zR+zL)/2<%lf cut",chi2[i]),"","");
...
//--------------Z vertex---------------//
	...
	h_L_vz->Fill((R_tr_vz[rt]+L_tr_vz[lt])/2);
	...
	//if(zcut){h_zz->Fill(R_tr_vz[rt],L_tr_vz[lt]);}
	h_zz_test->Fill(R_tr_vz[rt],L_tr_vz[lt]);
	if(fabs(R_tr_vz[rt]-0.12)<0.02 && fabs(L_tr_vz[lt]+0.12)<0.02){h_zz1->Fill(ct,mm);h_z1->Fill(ct);}
	...
	if(fabs(L_tr_vz[lt])<0.1 && fabs(R_tr_vz[rt]-0.12)<0.02){h_zz4->Fill(ct,mm);h_z4->Fill(ct);}
				...
				//zcut=false;
				//if(chi2cut && fabs(R_tr_vz[rt]-L_tr_vz[lt])<0.025 && fabs(R_tr_vz[rt] + L_tr_vz[lt])/2.0<0.1)zcut=true;
				
    if(k%100000==0){
	cout<<"R_tr_chi2[rt]="<<R_tr_chi2[rt]<<endl;
	cout<<"L_tr_chi2[lt]="<<L_tr_chi2[lt]<<endl;
	cout<<"R_tr_vz[rt]="<<R_tr_vz[rt]<<endl;
	cout<<"L_tr_vz[lt]="<<L_tr_vz[lt]<<endl;
	cout<<"zcut:"<<zcut<<endl;
	cout<<"cut_ac1:"<<cut_ac1<<endl;
	cout<<"cut_ac2:"<<cut_ac2<<endl;
	}
...
if(i==50){
	...
	bool chi2cut=false;
        //if( R_tr_chi2[rt]<(chi2[i]/5000) && L_tr_chi2[lt]<(chi2[i]/5000) )chi2cut=true;
        if( fabs(R_tr_chi2[rt]-0.002)<chi2[i] && fabs(L_tr_chi2[lt]-0.002)<chi2[i] )chi2cut=true;

		
	if(tr.AC1_npe_sum<3.75 && tr.AC2_npe_sum>3. && tr.AC2_npe_sum<20. && zcut && chi2cut){
	if(i==5){h_zz->Fill(R_tr_vz[rt],L_tr_vz[lt]);}
...
//-----No AC Cut-----//
 ...
 fp_noZ->SetNpx(2000);
 fpi_noZ =new TF1("fpi_noZ","gausn(0)",min_coin_c,max_coin_c);
 fpi_noZ->SetNpx(2000);
 fk_noZ=new TF1("fk_noZ","gausn(0)",min_coin_c,max_coin_c);
 fk_noZ->SetNpx(2000);
//cout<<"fp fit start"<<endl;
 hcoin_wo_bg_fom_noZ->Fit("fp_noZ","Rq","0",def_mean_p-3*def_sig_p,def_mean_p+3*def_sig_p);
...
//cout<<"fk fit start"<<endl;
 ...
 hcoin_wo_bg_fom_noZ->Fit("fk_noZ","Rq","0",-1,1);
...
// mean_k_noZ=0;
 ...
 n_k_noZ=hcoin_k_fom_noZ->Integral(hcoin_wo_bg_fom_noZ->FindBin(center_k-range_k),hcoin_wo_bg_fom_noZ->FindBin(center_k+range_k));
	...
	cout<<"range_k(No Cut)="<<range_k<<endl;
 ...
 //hcoin_wo_bg_fom_noZ->Fit("fcoin_woAC","Rq","0",min_coin_c,max_coin_c);
...
// hmm_bg_fom_noZ->Fit("fmmbg_noZ","Rq","",min_mm,max_mm);
// hmm_pi_wobg_fom_noZ->Fit("fmmbg_noZ","Rq","",-0.05,0.15);
...
// fmm_noZ->SetParameters(9,100);
 ...
 hmm_wo_bg_fom_noZ->Fit("fmm_noZ","","",-0.05,0.15);
...
// hmm_wo_bg_fom_noZ->Fit("fL_noZ","Rq","",def_mean_L-3*def_sig_L,def_mean_L+3*def_sig_L);
...
// hmm_wo_bg_fom_noZ->Fit("fS_noZ","Rq","",def_mean_S-3*def_sig_S,def_mean_S+3*def_sig_S);
...
// n_k[i][j][l]=fk[i][j][l]->GetParameter(0);
 ...
 n_k[i][j][l]=hcoin_k_fom[i][j][l]->Integral(hcoin_wo_bg_fom[i][j][l]->FindBin(center_k-range_k),hcoin_wo_bg_fom[i][j][l]->FindBin(center_k+range_k));
	cout<<"center_k="<<center_k<<endl;
...
//----------------------------------------------//
	...
	if(n_pi[i][j][l]>n_pi_noZ){
		for(int fill=0;fill<n_pi_noZ;fill++){h_pisr1->Fill(chi2[i]+0.00005);}
	}
	else {for(int fill=0;fill<n_pi[i][j][l];fill++){h_pisr1->Fill(chi2[i]+0.00005);}}

	if(n_k[i][j][l]>n_k_noZ){
		for(int fill=0;fill<n_k_noZ;fill++){h_ksr1->Fill(chi2[i]+0.00005);}
	}
	else {for(int fill=0;fill<n_k[i][j][l];fill++){h_ksr1->Fill(chi2[i]+0.00005);}}
	if(n_p[i][j][l]>n_p_noZ){
		for(int fill=0;fill<n_p_noZ;fill++){h_psr1->Fill(chi2[i]+0.00005);}
	}
	else {for(int fill=0;fill<n_p[i][j][l];fill++){h_psr1->Fill(chi2[i]+0.00005);}}

	if(n_L[i][j][l]>n_L_noZ){
		for(int fill=0;fill<n_L_noZ;fill++){h_Lsr1->Fill(chi2[i]+0.00005);}
	}
	else {for(int fill=0;fill<n_L[i][j][l];fill++){h_Lsr1->Fill(chi2[i]+0.00005);}}
	if(n_S[i][j][l]>n_S_noZ){
		for(int fill=0;fill<n_S_noZ;fill++){h_Ssr1->Fill(chi2[i]+0.00005);}
	}
	else {for(int fill=0;fill<n_S[i][j][l];fill++){h_Ssr1->Fill(chi2[i]+0.00005);}}

	for(int fill=0;fill<n_pi_noZ;fill++) h_pitot1->Fill(chi2[i]+0.00005);
	for(int fill=0;fill<n_k_noZ;fill++) h_ktot1->Fill(chi2[i]+0.00005);
	for(int fill=0;fill<n_p_noZ;fill++) h_ptot1->Fill(chi2[i]+0.00005);
	for(int fill=0;fill<n_L_noZ;fill++) h_Ltot1->Fill(chi2[i]+0.00005);
	for(int fill=0;fill<n_S_noZ;fill++) h_Stot1->Fill(chi2[i]+0.00005);
...
//ofstream fout(Form("SR_chi2_%d.dat",(int)chi2[0]));
ofstream fout("SR_chisq.dat");
cout<<"Chi2 Efficiency is filled."<<endl;
//fout<<n_pi_noZ<<" "<<n_k_noZ<<" "<<n_p_noZ<<" "<<n_L_noZ<<" "<<n_S_noZ<<endl;
for(int i=0;i<nth;i++){
//	for(int j=0;j<nth;j++){
		int j=0;	int l=0;
		fout<<n_pi[i][j][l]/n_pi_noZ<<" "<<n_k[i][j][l]/n_k_noZ<<" "<<n_p[i][j][l]/n_p_noZ<<" "<<n_L[i][j][l]/n_L_noZ<<" "<<n_S[i][j][l]/n_S_noZ<<" chi2<"<<chi2[i] <<endl;
//	}
}
...
h_z4->Draw("");
c10->cd();
h_ktot1->Draw("");h_ksr1->SetLineColor(kRed);h_ksr1->Draw("same");
c11->cd();
h_Ltot1->Draw("");h_Lsr1->SetLineColor(kRed);h_Lsr1->Draw("same");
c12->cd()->DrawFrame(0.,0.,100.,1.2);//K,L,S
cout<<"c12 start"<<endl;
...
c13->cd()->DrawFrame(0.,0.,100.,1.2);//L,S
cout<<"c13 start"<<endl;
pEff4->SetLineColor(kAzure);pEff4->Draw("same");
pEff5->SetLineColor(kCyan);pEff5->Draw("same");
c14->cd()->DrawFrame(0.,0.,100.,1.2);//pi,k,p
cout<<"c14 start"<<endl;
...
cout<<"n_pi:mean_pi:sig_pi"<<n_pi[99][0][0]<<" : "<<mean_pi[99][0][0]<<" : "<<sig_pi[99][0][0]<<endl;
cout<<"n_k:mean_k:sig_k"<<n_k[99][0][0]<<" : "<<mean_k[99][0][0]<<" : "<<sig_k[99][0][0]<<endl;
cout<<"n_p:mean_p:sig_p"<<n_p[99][0][0]<<" : "<<mean_p[99][0][0]<<" : "<<sig_p[99][0][0]<<endl;
cout<<"n_L:mean_L:sig_L"<<n_L[99][0][0]<<" : "<<mean_L[99][0][0]<<" : "<<sig_L[99][0][0]<<endl;
cout<<"n_S:mean_S:sig_S"<<n_S[99][0][0]<<" : "<<mean_S[99][0][0]<<" : "<<sig_S[99][0][0]<<endl;
}
//////////////////////////////////////////////////////////
