...
double explandau(double *x, double *par, int num) {
  //par[0]=area? width?
  //par[1]=tau of exp function
  //par[2]=Width (sigma) of convoluted Gaussian function
  //par[3]=Shift of Function Peak
...
// Convolution integral
  for(i=1.0; i<=np/2; i++){
     xx = xlow + (i-0.5) * step - par[num+3];
     fland = TMath::Landau(xx,x[0],par[num+2]) / par[num];
     sum += fland * TMath::Exp(-xx/par[num+1]);
     xx = xupp - (i-.5) * step - par[num+3];
     fland = TMath::Landau(xx,x[0],par[num+2]) / par[num];
     sum += fland * TMath::Exp(-xx/par[num+1]);
  ...
  //val = par[2] * step * sum * invsq2pi / par[3];
  val = par[num+4] * step * sum * invsq2pi / (par[num+2]*par[num+1]*exp(par[num+3]/par[num+1]));
  return val;
}
...
double FMM_Res( double *x, double *par ){
	...
	return landaugaus2(x,par,0)+landaugaus2(x,par,4)+explandau(x,par,8)+explandau(x,par,13);//+expgaus2(x,par,14);

}

double FMM_Lambda_only( double *x, double *par ){

	return landaugaus2(x,par,0)+explandau(x,par,4);

}

double FMM_Sigma_only( double *x, double *par ){
...
/*%%%%%%%%%%%%%%%%%%%%%%%%*/
	//--- w/ 4th Polynomial func.
	 ...
	 fmm_best_4Poly=new TF1("fmm_best_4Poly",FMM_Res,fit_min_mm,fit_max_mm,18);
   //par[0]=Width (scale) parameter of Landau density
   //par[1]=Most Probable (MP, location) parameter of Landau density
   //par[2]=Total area (integral -inf to inf, normalization constant)
   //par[3]=Width (sigma) of convoluted Gaussian function
	 ...
	 fmm_best_4Poly->SetParLimits(0,0.001,0.01);
	 ...
	 fmm_best_4Poly->SetParLimits(4,0.001,0.01);
...
//subL
  ...
  //par[3]=Shift of Function Peak
  //par[4]=Normalization
	 fmm_best_4Poly->SetParameter(8,0.002);//scale
	 fmm_best_4Poly->SetParLimits(8,0.,1.5);
	 fmm_best_4Poly->SetParameter(9,0.05);//att.
	 fmm_best_4Poly->SetParLimits(9,0.005,0.08);
	 fmm_best_4Poly->SetParameter(10,0.002);//sigma
	 fmm_best_4Poly->SetParLimits(10,0.001,0.1);
	 fmm_best_4Poly->FixParameter(11,0.);//peak pos.
	 fmm_best_4Poly->SetParameter(12,0.01);//scale
	 //fmm_best_4Poly->SetParLimits(11,-0.05,0.05);
//Sigma0
	 fmm_best_4Poly->SetParameter(13,0.002);
	 fmm_best_4Poly->SetParLimits(13,0.,1.5);
	 fmm_best_4Poly->SetParameter(14,0.05);
	 fmm_best_4Poly->SetParLimits(14,0.04,0.12);
	 fmm_best_4Poly->SetParameter(15,0.001);
	 fmm_best_4Poly->SetParLimits(15,0.001,0.01);
	 fmm_best_4Poly->FixParameter(16,-1.*def_mean_S);
	 fmm_best_4Poly->SetParameter(17,0.002);//scale
...
//	 fmm_best_4Poly->SetParLimits(19,-0.002,0.002);
	 ...
	 TF1* fmm_Lambda_only = new TF1("fmm_Lambda_only",FMM_Lambda_only,fit_min_mm,fit_max_mm,9);
	 TF1* fmm_Sigma_only  = new TF1("fmm_Sigma_only" ,FMM_Sigma_only, fit_min_mm,fit_max_mm,9);
//Lambda_only
	 fmm_Lambda_only->SetNpx(2000);
	 ...
	 fmm_Lambda_only->SetParameter(4,fmm_best_4Poly->GetParameter(8));
	 fmm_Lambda_only->SetParameter(5,fmm_best_4Poly->GetParameter(9));
	 fmm_Lambda_only->SetParameter(6,fmm_best_4Poly->GetParameter(10));
	 fmm_Lambda_only->SetParameter(7,fmm_best_4Poly->GetParameter(11));
	 fmm_Lambda_only->SetParameter(8,fmm_best_4Poly->GetParameter(12));
//Sigma_only
	 fmm_Sigma_only->SetNpx(2000);
	 fmm_Sigma_only->SetParameter(0,fmm_best_4Poly->GetParameter(4));
	 fmm_Sigma_only->SetParameter(1,fmm_best_4Poly->GetParameter(5));
	 fmm_Sigma_only->SetParameter(2,fmm_best_4Poly->GetParameter(6));
	 fmm_Sigma_only->SetParameter(3,fmm_best_4Poly->GetParameter(7));
	 fmm_Sigma_only->SetParameter(4,fmm_best_4Poly->GetParameter(13));
	 fmm_Sigma_only->SetParameter(5,fmm_best_4Poly->GetParameter(14));
	 fmm_Sigma_only->SetParameter(6,fmm_best_4Poly->GetParameter(15));
	 fmm_Sigma_only->SetParameter(7,fmm_best_4Poly->GetParameter(16));
	 fmm_Sigma_only->SetParameter(8,fmm_best_4Poly->GetParameter(17));
