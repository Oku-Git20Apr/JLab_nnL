...
//
//K. Okuyama (June 16, 2022)
//-- estimation of systematic error for choosing functions' parameters
//-- especially when dividing the data by Q2 and/or Angle
//-- but, these results are not used because the errors are negligibly small.
//
//taken over from simc_fitting2022.C
...
//Fix Fit (Syst. error for division)
#if 1 //from simc_fitting2022.C
...
//change	 fmm_strict_Lexp->SetParLimits(21,0.0,0.2);
	...
	hmm_simc2->Draw("same");
	 hmm_simc2->Fit("fmm_strict_Lexp","I","",fit_min_mm,fit_max_mm);//Total fitting (full) w/ 4Poly BG
...
//
	...
	cout<<"Number of Lambda (TF1 Integral) = "<<nofL_strict<<endl;
	cout<<"Number of Lambda in SIMC (TH1 Integral) = "<<nofLonly_simc<<endl;
	cout<<"Systemtic error of Lambda func. = " << abs(nofL_strict-nofLonly_simc)/nofLonly_simc<<endl;

	double nofSonly_simc=hmm_simcS2->Integral();
	cout<<"Number of Sigma (TF1 Integral) = "<<nofS_strict<<endl;
	cout<<"Number of Sigma in SIMC (TH1 Integral) = "<<nofSonly_simc<<endl;
	cout<<"Systemtic error of Sigma func. = " << abs(nofS_strict-nofSonly_simc)/nofSonly_simc<<endl;
