...
//Chapter 3
//
//K. Okuyama (Nov. 12, 2020)
//
//This is taken over from vertex_fit2.C
...
//  TH1F* hmm_bg_fom_best  = new TH1F("hmm_bg_fom_best","hmm_bg_fom_best",xbin,xmin,xmax);
  ...
  TH2F* h_zL_Y    = new TH2F("h_zL_Y"   ,""   , 200,-15.,15.,200,  -8., 8.); 
  h_zL_Y->GetXaxis()->SetTitle("Z-vertex [cm]");
  h_zL_Y->GetYaxis()->SetTitle("Y(FP) [cm]");
  h_zL_Y->GetYaxis()->SetTitleOffset(0.965);
  //h_zL_Y->GetZaxis()->SetLabelOffset(-0.005);
  TH2F* h_zR_Y    = new TH2F("h_zR_Y"   ,""   , 200,-15.,15.,200,  -8., 8.); 
  h_zR_Y->GetXaxis()->SetTitle("(-1)*Z-vertex [cm]");
  ...
  h_zz->GetYaxis()->SetTitleOffset(0.965);
...
//		//if(event_selection_nocut&&ct_cut)hcs_L_fom_nocut->SetBinContent(hmm_L_fom_best->FindBin(mm),cs);
		...
		h_zL_Y->Fill(L_tr_vz*100.,L_tr_y*100.);
		h_zR_Y->Fill(-1.*R_tr_vz*100.,-1.*R_tr_y*100.);
		}
		}
	...
	c2->SetLogy(1);
	 ...
	 TF1* fz_corr  = new TF1("fz_corr","F_VZ2",-0.15,0.15,11);
	 TF1* fz_cell  = new TF1("fz_cell","F_VZ_cell",-0.15,0.15,8);
//-------------------------------//
//    NEW Z FUNCTION             //
//-------------------------------//
//par[0]: Al front scale
...
//	h_zave->Fit("fz_corr","","",-0.15,0.15);
	...
	c7->Divide(1,1,1E-4,1E-4);
	//c7->SetRightMargin(+0.3);
	gPad->SetLeftMargin(0.15);
	gPad->SetRightMargin(0.15);
	gPad->SetTopMargin(0.15);
	gPad->SetBottomMargin(0.15);
