//-- Cointime  --//
//Itabashi type vs Gogami type
//
//Appendix
//
//K. Okuyama (Nov. 30, 2020)
//
...
void F1TDC(){
  ...
  TFile *file = new TFile("../h2all_1_2020Nov.root","read");//2020Nov updated, h2all_2(latter-half) was wrong for ct_itabashi
...
//---------------------------------------//
 ...
 double ct, ct_eff, ct_ita;
	...
	tree->SetBranchStatus("tr.ntrack_r",1);tree->SetBranchAddress("tr.ntrack_r",&NRtr);
  	...
  	tree->SetBranchStatus("ct_itabashi",1);  tree->SetBranchAddress("ct_itabashi", &ct_ita);
...
//  TH1F* hmm_bg_fom_best  = new TH1F("hmm_bg_fom_best","hmm_bg_fom_best",xbin,xmin,xmax);
  ...
  TH1F* h_coin_itabashi  = new TH1F("h_coin_itabashi","",40.*1000./56.,-20.,20.);
  h_coin_itabashi->GetXaxis()->SetTitle("Coincidence Time [ns]");
  h_coin_itabashi->GetYaxis()->SetTitle("Counts");
  h_coin_itabashi->SetLineColor(kAzure);
  TH1F* h_coin_gogami  = new TH1F("h_coin_gogami","",40.*1000./56.,-20.,20.);
  h_coin_gogami->GetXaxis()->SetTitle("Coincidence Time [ns]");
  h_coin_gogami->GetYaxis()->SetTitle("Counts");
  h_coin_gogami->SetLineColor(kAzure);
...
//		//if(event_selection_nocut&&ct_cut)hcs_L_fom_nocut->SetBinContent(hmm_L_fom_best->FindBin(mm),cs);
		...
		if(R_Tr&&R_FP&&L_Tr&&L_FP){
		h_coin_gogami->Fill(ct);
		h_coin_itabashi->Fill(ct_ita);
		}
	...
	h_coin_itabashi->Draw("");
	cout<<"Coin_all_ENum="<<h_coin_itabashi->Integral(h_coin_itabashi->FindBin(-20.),h_coin_itabashi->FindBin(20.))<<endl;
	TCanvas* c2 = new TCanvas("c2","c2");
	h_coin_gogami->Draw("");
	cout<<"Coin_kaon_ENum="<<h_coin_strict->Integral(h_coin_strict->FindBin(-1.006),h_coin_strict->FindBin(1.006))<<endl;
	
	c1->Print("./pdf/cointime_itabashi.pdf");
	c2->Print("./pdf/cointime_gogami.pdf");
