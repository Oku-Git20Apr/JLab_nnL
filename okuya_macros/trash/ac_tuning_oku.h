...
//////////////////////a1, a2, running parameters are three, so nth = 3 is plausible.
char* mode="H";
int kine=1;
...
////////////////Cointime////////////////////////////////// 
 facc_kc=new TF1("facc_kc","[0]",min_coin_c,max_coin_c); //BG, constant
 facc_kc->SetNpx(2000);
 fk_kc=new TF1("fk_kc","gausn(0)+pol0(3)",min_coin_c,max_coin_c);// kaon, gaus
 fk_kc->SetNpx(2000);
 fpi_pic=new TF1("fpi_pic","gausn(0)+pol0(3)",min_coin_c,max_coin_c);// pion, gaus
 fpi_pic->SetNpx(2000);
 fp_pc=new TF1("fp_pc","gausn(0)+pol0(3)",min_coin_c,max_coin_c);// proton, gaus
 fp_pc->SetNpx(2000);
 
////////////////Missing Mass////////////////////////////////// 
  fLam_p=new TF1("fLam_p","gausn(0)",min_mm,max_mm);// Lambda, gaus
      fLam_p->SetNpx(2000);
      fLam_p->SetLineColor(4);
      fLam_p->SetFillColor(4);
      fLam_p->SetFillStyle(3001);  
  fSig_p=new TF1("fSig_p","gausn(0)",min_mm,max_mm);// Sigma, gaus
      fSig_p->SetNpx(2000);
...
void tuningAC::Fill(){
 ...
 int ev=0;
 double ct;
 for(int k=0;k<ENum;k++){//go to :1490
   T->GetEntry(k);
...
cout<<"defined parameters in SR analysis"<<endl;
 ...
 def_acc_k=facc_kc->GetParameter(0);//BG constant is determined
...
/////////////////////////////////////////////////////////////////////BG, k, p, pi are fitted, and we got parameters.

 //============= SRA ======================//

     for(int th1=0;th1<nth;th1++){
     for(int th2=0;th2<nth;th2++){
     if( th1==th2){       /////th1=th2=0,1,2
     for(int i=0;i<iter_ac1;i++){

    th_ac1[i]=th1_max-(th1_max-min_ac1)/iter_ac1*i;//min_ac1 ~ th1_max: divide by iter_ac1
    th_ac2[i]=min_ac2+(th2_max-min_ac2)/iter_ac1*i;//min_ac2 ~ th2_max: divide by iter_ac1
...
//change
  hcoin_ac1_p[i][th2]->Add(hcoin_ac1_all_p[i][th2], hcoin_ac1_acc_p[i][th2],1.0,-1.0);//hcoin_all - hcoin_acc
...
//change
//	string ofname_tmp = "../test.pdf";
//	c11=new TCanvas("c11","c11");
//	c11->cd();
//	//hmm_fom->Draw();
//	hcoin_ac1_all_p[0][0]->Draw();
//
//  cout<<"Print is starting"<<endl;
//  cout<<"pdf name: "<<ofname_tmp<<endl;
// c11->Print(Form("%s[",ofname_tmp.c_str()));
// c11->Print(Form("%s",ofname_tmp.c_str()));
//
//
////	hmm_fom->SetName("test");
////	hmm_fom->Write();
//	hcoin_ac1_all_p[0][0]->SetName("test");
//	hcoin_ac1_all_p[0][0]->Write();
}//end Fitting
...
//  th_ac2_t=10000.; // FIll Cut of hmm_ac1 (Upper cut)
   if(ac2_up && z_cut && Ra2sum<th_ac2_t)ac2_flag=true;

cout << " Ra2sum===" << Ra2sum << endl;
   if(ac2_down && z_cut && Ra2sum>th_ac2_b)ac2_flag=true;
//   if(ac2_up && z_cut
...
//======== Draw TCanvas ==============//
  ...
  cout<<"aaaaaaaaaaaaa"<<endl;
   ...
   c15->Divide(4,4);
   for(int i=0;i<16;i++){
  cout<<"aaaaaaaaaaaaa"<< iter_ac1 << endl;
    if(iter_ac1<16) l=i;
 ...
 if(i==0)gsum_k_ac2[i][i]->Draw("AP");
  ...
  cout<<"c30 is finished"<<endl;
	
      c32=new TCanvas("c32","AC hist");
      c32->cd();
      hAC->Draw("colz");

  cout<<"c32 is finished"<<endl;

      c34=new TCanvas("c34","AC hist");
      c34->cd();
      hmm_ac1_all_p[fom_th1][fom_max_th2]->SetLineColor(1); hmm_ac2_all_p[fom_th2][fom_max_th1]->SetLineColor(4);
