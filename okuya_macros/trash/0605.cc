...
//Okuyama		    	//if(ac2l_adc[j]<R_a2_asum_p && R_a2_asum_p < ac2u_adc[l])cut_ac2=true;
		    	...
		    	if(ac2l_adc[j]<tr.AC2_npe_sum)cut_ac2=true;
//Changed			
...
// fmmbg_noAC->SetParameter(2,0.03);
 ...
 fmm_noAC->SetParLimits(3,0.,100.);//positive
 fmm_noAC->SetParLimits(4,0.,100000.);//positive
 fmm_noAC->SetParLimits(7,0.,100000.);//positive
...
// hmm_bg_fom_noAC->Fit("fmmbg_noAC","Rq","",min_mm,max_mm);
 hmm_pi_wobg_fom_noAC->Fit("fmmbg_noAC","Rq","",-0.1,0.15);
double a = fmmbg_noAC->GetParameter(0);
double b = fmmbg_noAC->GetParameter(1);
double c = fmmbg_noAC->GetParameter(2);
double d = fmmbg_noAC->GetParameter(3);
...
// fmm_noAC->SetParameters(0,a);
// fmm_noAC->SetParameters(1,b);
// fmm_noAC->SetParameters(2,c);
// fmm_noAC->SetParameters(3,d);
// fmm_noAC->SetParameters(4,e);
// fmm_noAC->SetParameters(5,f);
// fmm_noAC->SetParameters(6,500);
 fmm_noAC->SetParameter(5,def_mean_L);
 fmm_noAC->SetParameter(6,def_sig_L);
// fmm_noAC->SetParameters(9,100);
 fmm_noAC->SetParameter(8,def_mean_S);
 fmm_noAC->SetParameter(9,def_sig_S);
 hmm_wo_bg_fom_noAC->Fit("fmm_noAC","Rq","",-0.1,0.15);
//a = fmm_noAC->GetParameter(0);
//b = fmm_noAC->GetParameter(1);
//c = fmm_noAC->GetParameter(2);
//d = fmm_noAC->GetParameter(3);
//e = fmm_noAC->GetParameter(4);
//f = fmm_noAC->GetParameter(5);
...
cout<<"before(L):: "<<n_L_noAC<<endl;
 n_L_noAC-=fmmbg_noAC->Integral(center_L-range_L,center_L+range_L);
...
cout<<"before(S):: "<<n_S_noAC<<endl;
 n_S_noAC-=fmmbg_noAC->Integral(center_S-range_S,center_S+range_S);
...
// fmmbg[i][j][l]->SetParameter(2,0.03);
 ...
 fmm[i][j][l]->SetParLimits(3,0.,100.);//positive
 fmm[i][j][l]->SetParLimits(4,0.,100000.);//positive
 fmm[i][j][l]->SetParLimits(7,0.,100000.);//positive
...
// hmm_bg_fom[i][j][l]->Fit("fmmbg[i][j][l]","Rq","",min_mm,max_mm);
 hmm_pi_wobg_fom[i][j][l]->Fit("fmmbg[i][j][l]","Rq","",-0.1,0.15);
double acut = fmmbg[i][j][l]->GetParameter(0);
double bcut = fmmbg[i][j][l]->GetParameter(1);
double ccut = fmmbg[i][j][l]->GetParameter(2);
double dcut = fmmbg[i][j][l]->GetParameter(3);
cout<<"0:1:2:3:4:5="<<acut<<"::"<<bcut<<"::"<<ccut<<"::"<<dcut<<endl;//"::"<<e<<"::"<<f<<endl;
 fmm[i][j][l]->FixParameter(1,bcut);//mean
 fmm[i][j][l]->FixParameter(2,ccut);//sigma
 fmm[i][j][l]->FixParameter(3,dcut);//lg
...
// fmm[i][j][l]->SetParameters(6,500);
 fmm[i][j][l]->SetParameter(5,def_mean_L);
 fmm[i][j][l]->SetParameter(6,def_sig_L);
// fmm[i][j][l]->SetParameters(9,100);
 fmm[i][j][l]->SetParameter(8,def_mean_S);
 fmm[i][j][l]->SetParameter(9,def_sig_S);
 hmm_wo_bg_fom[i][j][l]->Fit("fmm[i][j][l]","Rq","",-0.1,0.15);
...
c3->cd(6);
hmm_L_fom[0][50][0]->Draw("");hmm_bg_fom[0][50][0]->SetLineColor(kRed);hmm_bg_fom[0][50][0]->Draw("same");
//c3->Divide(2,2);
//cout<<"c3 start"<<endl;
//c3->cd(1);
...
c4->cd(1);
h_L_vz->Draw("");
double vzLmax = (h_L_vz->GetBinContent(h_L_vz->GetMaximumBin()));
TLine *vz1, *vz2;
...
c4->cd(2);
h_R_vz->Draw("");
double vzRmax = (h_R_vz->GetBinContent(h_R_vz->GetMaximumBin()));
TLine *vz3, *vz4;
