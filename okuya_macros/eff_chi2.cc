...
//   max_coin_c=1000.0;
 ...
 max_adc=20000.;
	...
	//=== AC Threshold variable ===//
     ...
     //----------------NPE---------------//
	 chi2[0]=0.;
	 for(int i=1; i<nth; i++) chi2[i]=chi2[i-1]+1.;
	 zver[0]=0.000;
...
if(i==50){
	...
	bool chi2cut=false;
        ...
        if( fabs(R_tr_chi2[rt]-0.002)<(chi2[i]/10000) && fabs(L_tr_chi2[lt]-0.002)<(chi2[i]/10000) )chi2cut=true;
...
//----------------------------------------------//
	...
	if(n_pi[i][j][l]>n_pi_noZ){
		for(int fill=0;fill<n_pi_noZ;fill++){h_pisr1->Fill(chi2[i]);}
	}
	else {for(int fill=0;fill<n_pi[i][j][l];fill++){h_pisr1->Fill(chi2[i]);}}

	if(n_k[i][j][l]>n_k_noZ){
		for(int fill=0;fill<n_k_noZ;fill++){h_ksr1->Fill(chi2[i]);}
	}
	else {for(int fill=0;fill<n_k[i][j][l];fill++){h_ksr1->Fill(chi2[i]);}}
	if(n_p[i][j][l]>n_p_noZ){
		for(int fill=0;fill<n_p_noZ;fill++){h_psr1->Fill(chi2[i]);}
	}
	else {for(int fill=0;fill<n_p[i][j][l];fill++){h_psr1->Fill(chi2[i]);}}

	if(n_L[i][j][l]>n_L_noZ){
		for(int fill=0;fill<n_L_noZ;fill++){h_Lsr1->Fill(chi2[i]);}
	}
	else {for(int fill=0;fill<n_L[i][j][l];fill++){h_Lsr1->Fill(chi2[i]);}}
	if(n_S[i][j][l]>n_S_noZ){
		for(int fill=0;fill<n_S_noZ;fill++){h_Ssr1->Fill(chi2[i]);}
	}
	else {for(int fill=0;fill<n_S[i][j][l];fill++){h_Ssr1->Fill(chi2[i]);}}

	for(int fill=0;fill<n_pi_noZ;fill++) h_pitot1->Fill(chi2[i]);
	for(int fill=0;fill<n_k_noZ;fill++) h_ktot1->Fill(chi2[i]);
	for(int fill=0;fill<n_p_noZ;fill++) h_ptot1->Fill(chi2[i]);
	for(int fill=0;fill<n_L_noZ;fill++) h_Ltot1->Fill(chi2[i]);
	for(int fill=0;fill<n_S_noZ;fill++) h_Stot1->Fill(chi2[i]);

cout<<"chi2: "<<chi2[i]/5000.<<endl;
...
//	for(int j=0;j<nth;j++){
		int j=0;	int l=0;
		fout<<n_pi[i][j][l]/n_pi_noZ<<" "<<n_k[i][j][l]/n_k_noZ<<" "<<n_p[i][j][l]/n_p_noZ<<" "<<n_L[i][j][l]/n_L_noZ<<" "<<n_S[i][j][l]/n_S_noZ<<" chi2<"<<chi2[i]/10000. <<endl;
//	}
}
