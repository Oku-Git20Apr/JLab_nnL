...
void simc_fitting2023(){
  ...
  TFile *file_mea = new TFile("/data/41a/ELS/okuyama/JLab_nnL/okuya_macros/MixedEventAnalysis/bgmea_llccrr_new_2023.root","read");//2023/10
...
//change
	...
	//
	hmm_simcL->Scale(836.434/382755.);//2023//Full
	hmm_simcS->Scale(285.716*285.716/309.647/185167.);//2023//Full
...
//2023/10
	 ...
	 fmm_strict_Lexp->SetParameter(3,0.00146711);//sigma
	 ...
	 fmm_strict_Lexp->SetParameter(10,0.00140370);//sigma
...
//change	 fmm_strict_Lexp->SetParLimits(21,0.0,0.2);
	...
	fmm_strict_Lexp->Draw("");
	 TH1F* h_Resulting  = new TH1F("h_Resulting","h_Resulting",xbin,xmin*0.001,xmax*0.001);
	 double val, eval;
	 for(int i=0;i<xbin;i++){
		val  = hmm_simc2->GetBinContent(i+1);
		eval = hmm_simc2->GetBinError(i+1);
		if(val>=0){
		h_Resulting->SetBinContent(i+1,val);
		h_Resulting->SetBinError(i+1,eval);
		}else{continue;}
	 }//Filling to the resulting histogram for chi2 fitting
	 h_Resulting->Draw("same");
	 h_Resulting->Fit("fmm_strict_Lexp","","",fit_min_mm,fit_max_mm);//Total fitting (full) w/ 4Poly BG
...
//bg_only_strict
	...
	TCanvas *c11 = new TCanvas("c11","c11",800,800);
	 TH1F* h_ResultingL  = new TH1F("h_ResultingL","h_ResultingL",xbin,xmin*0.001,xmax*0.001);
	 val=0;eval=0;
	 for(int i=0;i<xbin;i++){
		val  = hmm_simcL2->GetBinContent(i+1);
		eval = hmm_simcL2->GetBinError(i+1);
		if(val>=0){
		h_ResultingL->SetBinContent(i+1,val);
		h_ResultingL->SetBinError(i+1,eval);
		}else{continue;}
	 }//Filling to the resulting histogram for chi2 fitting
	 h_ResultingL->Draw("");
	 fmm_Lambda_only_strict->Draw("same");
	 //hmm_simcL2->Fit("fmm_Lambda_only_strict","I","",fit_min_mm,fit_max_mm);//Lambda fitting
	 h_ResultingL->Fit("fmm_Lambda_only_strict","I","",-0.001,0.07);//Lambda fitting
	...
	double nofLonly_simc=h_ResultingL->Integral();
	...
	TCanvas *c12 = new TCanvas("c12","c12",800,800);
	 TH1F* h_ResultingS  = new TH1F("h_ResultingS","h_ResultingS",xbin,xmin*0.001,xmax*0.001);
	 val=0;eval=0;
	 for(int i=0;i<xbin;i++){
		val  = hmm_simcS2->GetBinContent(i+1);
		eval = hmm_simcS2->GetBinError(i+1);
		if(val>=0){
		h_ResultingS->SetBinContent(i+1,val);
		h_ResultingS->SetBinError(i+1,eval);
		}else{continue;}
	 }//Filling to the resulting histogram for chi2 fitting
	 h_ResultingS->Draw("");
	 fmm_Sigma_only_strict->Draw("same");
	 ...
	 h_ResultingS->Fit("fmm_Sigma_only_strict","I","",0.07,0.11);//Sigma fitting
	...
	double nofSonly_simc=h_ResultingS->Integral();
