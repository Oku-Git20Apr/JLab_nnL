...
//====K40====//
	...
	TH2D *h_costh_ph_eachmom[bin_mom];
	for(int i=0;i<bin_mom;i++) h_costh_ph_eachmom[i] = new TH2D( Form("h_costh_ph_eachmom[%d]",i),"", bin_2D_costh, min_costh, max_costh, bin_2D_ph, min_ph, max_ph);
	...
	TH1D *h_mom_result_fine[bin_2D_costh][bin_2D_ph];

	int n1, n2;
	double val;
	double err;
	int n4[bin_mom][bin_2D_costh][bin_2D_ph];
	double Gamma[bin_mom][bin_2D_costh][bin_2D_ph];//VP Flux for each momentum bin (before Integral)
	for(int i=0;i<bin_mom;i++){
		for(int j=0;j<bin_2D_costh;j++){
			for(int k=0;k<bin_2D_ph;k++){
			Gamma[i][j][k]=0.;
			n4[i][j][k]=0;
			if(i==0)h_mom_result_fine[j][k] = new TH1D( Form("h_mom_result_fine[%d][%d]",j,k), "", bin_mom, min_mom, max_mom);
			}
		}
	}
...
//cout<<"vpflux vs k = "<<vpflux<<" : "<<k<<endl;
	...
	//	if(mom>2.1&&mom<2.15){
	 ...
	 //if(vpflux>k&&fabs(th-0.225)<0.025&&fabs(ph)<0.25){//new top-quality (6msr)
		...
		//int nmom=(mom-min_mom)*bin_mom/(max_mom-min_mom); 
		int nmom=h_vp_mom->FindBin(mom); 
		//cout<<"nmom="<<nmom<<endl;
		h_mom_result_fine[h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))-1][h_costh_ph_eachmom[nmom-1]->GetYaxis()->FindBin(ph)-1]->Fill(mom);
		h_costh_ph_eachmom[nmom-1]->Fill(cos(th),ph);
		//Gamma[nmom-1][h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))-1][h_costh_ph_eachmom[nmom-1]->GetYaxis()->FindBin(ph)-1]=vpflux;

		//cout<<"Gamma["<<nmom-1<<"]["<<h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))-1<<"]["<<h_costh_ph_eachmom[nmom-1]->GetYaxis()->FindBin(ph)-1<<"] is filled!"<<endl;
		//if(mom<2.12){//Full (Sigma)
		h_vp_mom2->SetBinContent(h_vp_mom2->FindBin(mom),vpflux);
		//if(fabs(mom-2.125)<0.025){//new top-quality (Lambda)
		h_vp_th->Fill(th);
		 cout<<"old (mom,th)="<<mom<<", "<<th<<endl;
		if(vpflux>0.){
	     Einc  = 4.3;//[GeV]
	//---Minimum---//
	     Escat = min_mom+((max_mom-min_mom)/bin_mom)*(nmom-1);//[GeV]
	     theta = acos(min_costh+((max_costh-min_costh)/bin_2D_costh)*(h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))-1));	
	//---Maximum---//
	     //Escat = min_mom+((max_mom-min_mom)/bin_mom)*(nmom);//[GeV]
	     //theta = acos(min_costh+((max_costh-min_costh)/bin_2D_costh)*(h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))));	
		 cout<<"new (mom,th)="<<Escat<<", "<<theta<<endl;

	     Mp=0.9382720;//[GeV/c^2]
	     Qsq=2*Einc*Escat*(1-cos(theta));
	     deltaE = Einc - Escat;
	     q2=Qsq+deltaE*deltaE;
	     kg=deltaE-Qsq/(2*Mp);
	     eps=1/(1+2*(q2/Qsq)*tan(theta/2)*tan(theta/2));
	     Gamma[nmom-1][h_costh_ph_eachmom[nmom-1]->GetXaxis()->FindBin(cos(th))-1][h_costh_ph_eachmom[nmom-1]->GetYaxis()->FindBin(ph)-1]=Escat*kg/(137*2*PI*PI*Einc*Qsq*(1-eps));
		}
	 //}//Full_S
	 }//Full_L
	 //}//Top_S
...
for(int i=0;i<bin_mom;i++){
	for(int j=0;j<bin_2D_costh;j++){
		for(int k=0;k<bin_2D_ph;k++){
	//cout<<"Gamma["<<i<<"]["<<j<<"]["<<k<<"]="<<Gamma[i][j][k]<<endl;
		}
	}
}
...
double n1n2_tot=0.;
	for(int i=0; i<bin_mom; i++){
		...
		for(int j=0;j<bin_2D_costh;j++){
			for(int k=0;k<bin_2D_ph;k++){
				n3 += (int)h_costh_ph_eachmom[i]->GetBinContent(j+1,k+1);
				n4[i][j][k] = (int)h_costh_ph_eachmom[i]->GetBinContent(j+1,k+1);
			}
		}
		int n5 = 0;
		cout<<"nbin_mom:"<<i<<endl;
		cout<<"n1="<<n1<<endl;
		cout<<"n2="<<n2<<endl;
		cout<<"n3(VPFLUX!!!)="<<n3<<endl;
		cout<<"Gamma[i][0][0]="<<Gamma[i][0][0]<<endl;
		//if(n1!=0 && n2!=0)val = (Gamma[i][0][0]/n2)*omega*(1.0*n2/n1)/1000.;
		if(n1!=0 && n3!=0){
		for(int j=0;j<bin_2D_costh;j++){
			for(int k=0;k<bin_2D_ph;k++){
				n5 = h_mom_result_fine[j][k]->GetBinContent(i+1);
				double omega2 = omega*(1.0*n5/n1)/1000.;
				//val += (Gamma[i][j][k]/n3)*omega*(1.0*n3/n1)/1000.;
				//if(n4[i][j][k]!=0)val += (Gamma[i][j][k]/n4[i][j][k])*omega*(1.0*n4[i][j][k]/n1)/1000.;
				//if(n4[i][j][k]!=0)val += (Gamma[i][j][k]/n4[i][j][k])*((max_costh-min_costh)*(max_ph-min_ph))/((double)bin_2D_costh*(double)bin_2D_ph);
				if(n5!=0){
				val += (Gamma[i][j][k]) * omega2;
				double err_temp = (Gamma[i][j][k]) * omega2* sqrt(1./n5 + 1./n1 - 2./sqrt(1.*n1*n5));
				err += err_temp*err_temp;
				}
				if(i==76){if(n4[i][j][k]!=0)omega_test += 0.04292/((double)bin_2D_costh*(double)bin_2D_ph);}
		...
		cout<<"n1="<<n1<<endl;
		cout<<"n2="<<n2<<endl;
		if(n1!=0 && n2!=0 && i>(h_mom_result->FindBin(2.1)))val = (Gamma[i][0][0]/n2)*omega*(1.0*n2/n1)/1000.;
	...
	for(int i=0; i<bin_th; i++){
		...
		if(n1!=0 && n2!=0)val = Gamma[i][0][0]*omega*(1.0*n2/n1)/1000.;
...
cout<<"Omega_acpt="<<omega_test<<endl;
...
//	h_sa_costh_ph->Draw("samecolz");
	...
	TLine *RHRS_min = new TLine( 2.1, 0., 2.1, 1.*ymax);
	TLine *RHRS_max = new TLine( 2.22, 0., 2.22, 1.*ymax);
