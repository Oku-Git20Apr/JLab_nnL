...
//====K40====//
	...
	double val;
	double err;
	double Gamma[bin_mom];//VP Flux for each momentum bin (before Integral)
	for(int i=0;i<bin_mom;i++){Gamma[i]=0.;}
...
//cout<<"vpflux vs k = "<<vpflux<<" : "<<k<<endl;
	...
	//	if(mom>2.1&&mom<2.15){
	 ...
	 //if(vpflux>k&&fabs(th-0.225)<0.025&&fabs(ph)<0.25){//new top-quality (6msr)
		...
		Gamma[h_mom_result->FindBin(mom)-1]+=vpflux;
...
double n1n2_tot=0.;
	for(int i=0; i<bin_mom; i++){
		...
		cout<<"n1="<<n1<<endl;
		cout<<"n2="<<n2<<endl;
		if(n1!=0 && n2!=0)val = (Gamma[i]/n2)*omega*(1.0*n2/n1)/1000.;
		...
		vpflux_tot_mom_err+=err*bin_width;
		...
		cout<<"n1="<<n1<<endl;
		cout<<"n2="<<n2<<endl;
		if(n1!=0 && n2!=0 && i+1>(h_mom_result->FindBin(2.1)))val = (Gamma[i]/n2)*omega*(1.0*n2/n1)/1000.;
	...
	for(int i=0; i<bin_th; i++){
		...
		if(n1!=0 && n2!=0)val = Gamma[i]*omega*(1.0*n2/n1)/1000.;
